...
export default function Profile() {
  ...
  //formik doesnt support file support
  const onUpload = async e => {
    //get the first image and converting it into base64
    const base64 = await convertToBase64(e.target.files[0]) //as it is returning a promise we need to give await
    setFile(base64)
  }
        ...
        <div className={`${styles.glass} ${extend.glass}`} style={{ width: "45%" }}>
          ...
          <form className="py-1" onSubmit={formik.handleSubmit}>
            ...
            <div className="text-center py-4">
              <span className="text-gray-500">Come Back Later? <button className='text-red-500'>Log Out</button></span>
            </div>
          </form>
        </div>
      </div>
